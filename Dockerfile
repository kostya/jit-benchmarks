...
RUN curl -L 'https://downloads.python.org/pypy/pypy2.7-v7.3.4-linux64.tar.bz2' > l.tar.bz2 \
  && tar xjf l.tar.bz2 \
  && curl -L 'https://downloads.python.org/pypy/pypy2.7-v7.3.4-src.tar.bz2' > s.tar.bz2 \
  && tar xjf s.tar.bz2 \
  && rm *.tar.bz2 \
  && ln -sf /opt/pypy2.7-v7.3.4-linux64/bin/pypy /usr/bin/pypy2
...
RUN curl -L 'https://downloads.python.org/pypy/pypy3.7-v7.3.4-linux64.tar.bz2' > l.tar.bz2 \
  && tar xjf l.tar.bz2 \
  && rm *.tar.bz2 \
  && ln -sf /opt/pypy3.7-v7.3.4-linux64/bin/pypy /usr/bin/pypy3
...
RUN curl https://bootstrap.pypa.io/pip/2.7/get-pip.py -o get-pip.py \
  && pypy2 get-pip.py \
  && rm get-pip.py \
  && ln -sf /opt/pypy2.7-v7.3.4-linux64/bin/pip /usr/bin/pypy2-pip

RUN git clone https://github.com/kostya/topaz.git \
  && cd topaz \
  && pypy2-pip install -r requirements.txt \
  && pypy2 ../pypy2.7-v7.3.4-src/rpython/bin/rpython -Ojit targettopaz.py \
  && rm -rf /tmp/*
